...
// ── Step config ───────────────────────────────────────────────────────────────
// AUTO: Template → Period → Review  (no income step)
const AUTO_STEPS = [
...
// MANUAL: Format → Categories → Period & Income → Allocations → Review
const MANUAL_STEPS = [
        ...
        <Grid container spacing={1.75}>
            <Grid item xs={12} sm={6}>
                <Box onClick={() => onSelect('auto')} sx={{
                    ...
                    <Typography sx={{ fontSize: '0.75rem', color: SLATE, lineHeight: 1.55, mb: 1.5 }}>
                        Pick a predefined template style and we'll configure categories, format, and layout automatically. Just set your period range and you're done.
                    </Typography>
            ...
            <Grid item xs={12} sm={6}>
                <Box onClick={() => onSelect('manual')} sx={{
                    ...
                    <Typography sx={{ fontSize: '0.75rem', color: SLATE, lineHeight: 1.55, mb: 1.5 }}>
                        Walk through a step-by-step wizard to choose your own categories, period range, income, and exact budget amounts per category.
                    </Typography>
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
...
const PresetDropdown: React.FC<{
    ...
    <FormControl fullWidth size="small" sx={{ mb: 1.75 }}>
        <Select
            ...
            displayEmpty
            renderValue={v => v
                ? <Typography sx={{ fontSize: '0.85rem', color: NAVY, fontWeight: 600 }}>{v as string}</Typography>
                : <Typography sx={{ fontSize: '0.85rem', color: alpha(SLATE, 0.7) }}>Choose a template style…</Typography>}
...
const AutoStepTemplate: React.FC<{
    ...
    return (
        <Box>
            <Typography sx={{ fontSize: '0.72rem', fontWeight: 700, color: SLATE, textTransform: 'uppercase', letterSpacing: '0.07em', mb: 1 }}>
...
// Step 1 — Period range only (no income)
        ...
        <Box>
            ...
            <Grid container spacing={2} sx={{ mb: 2 }}>
                <Grid item xs={6}>
                    ...
                    <TextField fullWidth size="small" type="month" value={startMonth} onChange={e => onStart(e.target.value)} InputLabelProps={{ shrink: true }}
                ...
                <Grid item xs={6}>
                    ...
                    <TextField fullWidth size="small" type="month" value={endMonth} onChange={e => onEnd(e.target.value)} InputLabelProps={{ shrink: true }}
                               sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px', fontSize: '0.85rem' } }} />
                </Grid>
            </Grid>
            {np > 0 && (
                <Box sx={{ p: 1.25, borderRadius: '8px', bgcolor: alpha(TEAL, 0.05), border: `0.5px solid ${alpha(TEAL, 0.2)}`, fontSize: '0.75rem', color: '#0f766e', fontWeight: 600 }}>
                    {np} {format.toLowerCase()} period{np !== 1 ? 's' : ''} will be generated
                </Box>
            )}
...
// Step 2 — Review (auto)
const AutoStepReview: React.FC<{
    preset: TemplatePreset; format: string;
    startMonth: string; endMonth: string; customName: string;
}> = ({ preset, format, startMonth, endMonth, customName }) => {
    const np          = calcNumPeriods(format, startMonth, endMonth);
    ...
    return (
        <Box>
            ...
            {preset.suggestedCategories.length > 0 && (
                <>
                    <Typography sx={{ fontSize: '0.72rem', fontWeight: 700, color: SLATE, textTransform: 'uppercase', letterSpacing: '0.07em', mb: 0.875 }}>
                        Categories ({preset.suggestedCategories.length})
                    </Typography>
            ...
            <Box sx={{ p: 1.25, borderRadius: '8px', bgcolor: alpha(TEAL, 0.05), border: `0.5px solid ${alpha(TEAL, 0.2)}`, fontSize: '0.75rem', color: '#0f766e' }}>
                Category rows will be created from the preset. All cells start blank — fill in your actuals as you go.
            </Box>
        </Box>
    );
};
...
const LAYOUT_OPTIONS = [
    ...
    },
    {
        ...
        desc: 'Categories organized under labeled section headers — useful for many related items.',
    ...
    return (
        <Box>
            ...
            {state.categories.length === 0 && (
                <Box sx={{ mt: 1.5, p: 1, borderRadius: '8px', bgcolor: alpha(AMBER, 0.07), border: `0.5px solid ${alpha(AMBER, 0.3)}`, fontSize: '0.75rem', color: '#92400e' }}>
    ...
    return (
        <Box>
            <Grid container spacing={2} sx={{ mb: 2 }}>
                <Grid item xs={6}>
                    <Typography sx={{ fontSize: '0.72rem', fontWeight: 700, color: SLATE, textTransform: 'uppercase', letterSpacing: '0.07em', mb: 0.75 }}>Start month</Typography>
                    <TextField fullWidth size="small" type="month" value={state.startMonth} onChange={e => update({ startMonth: e.target.value })} InputLabelProps={{ shrink: true }}
                               sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px', fontSize: '0.85rem' } }} />
                </Grid>
                <Grid item xs={6}>
                    <Typography sx={{ fontSize: '0.72rem', fontWeight: 700, color: SLATE, textTransform: 'uppercase', letterSpacing: '0.07em', mb: 0.75 }}>End month</Typography>
                    <TextField fullWidth size="small" type="month" value={state.endMonth} onChange={e => update({ endMonth: e.target.value })} InputLabelProps={{ shrink: true }}
                               sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px', fontSize: '0.85rem' } }} />
                </Grid>
            </Grid>
            ...
            {state.startMonth && state.endMonth && new Date(state.endMonth + '-01') < new Date(state.startMonth + '-01') && (
                <Box sx={{ mt: 1.5, p: 1, borderRadius: '8px', bgcolor: alpha(RED, 0.06), border: `0.5px solid ${alpha(RED, 0.2)}`, fontSize: '0.75rem', color: RED }}>
                    End month must be after start month.
                </Box>
            )}
        </Box>
...
const StepAllocations: React.FC<{ state: WizardState; update: (p: Partial<WizardState>) => void }> = ({ state, update }) => {
    ...
    const autoFill = () => {
        ...
        state.categories.forEach(c => {
            if (!allocs[c.name] || allocs[c.name] === '0') {
                allocs[c.name] = (inc * (DEFAULT_ALLOC[c.name] ?? 0.08)).toFixed(2);
            }
        });
        update({ allocs });
    };
        ...
        <Box>
            {inc > 0 && (
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
                    <Typography sx={{ fontSize: '0.72rem', color: SLATE }}>
                        Income per period: <strong style={{ color: NAVY }}>${fmtMoney(inc)}</strong>
                    </Typography>
                    <Button onClick={autoFill} size="small" sx={{ fontSize: '0.72rem', fontWeight: 700, color: TEAL, textTransform: 'none', border: `0.5px solid ${alpha(TEAL, 0.3)}`, borderRadius: '6px', px: 1, py: 0.4, '&:hover': { bgcolor: alpha(TEAL, 0.06) } }}>
            ...
            <Box sx={{ p: 1.5, borderRadius: '10px', bgcolor: alpha(barColor, 0.05), border: `0.5px solid ${alpha(barColor, 0.25)}` }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.875 }}>
                    ...
                    <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: overBudget ? RED : GREEN }}>
                        {overBudget ? `$${fmtMoney(-remaining)} over` : `$${fmtMoney(remaining)} remaining`}
                    </Typography>
                </Box>
...
const StepReview: React.FC<{ state: WizardState }> = ({ state }) => {
    ...
    return (
        <Box>
            ...
            {over && (
                <Box sx={{ p: 1.125, borderRadius: '8px', bgcolor: alpha(AMBER, 0.07), border: `0.5px solid ${alpha(AMBER, 0.3)}`, fontSize: '0.75rem', color: '#92400e', mb: 1 }}>
                    Allocations exceed income — you can adjust values afterwards.
                </Box>
            )}
            <Box sx={{ p: 1.25, borderRadius: '8px', bgcolor: alpha(TEAL, 0.05), border: `0.5px solid ${alpha(TEAL, 0.2)}`, fontSize: '0.75rem', color: '#0f766e' }}>
...
const BLANK_WS: WizardState = {
    templateName: '', format: '', categories: [], startMonth: '', endMonth: '',
    income: '', allocs: {}, newCatName: '', groupByCategory: false,
};
...
export const ManualTemplateWizard: React.FC<ManualTemplateWizardProps> = ({ open, onClose, onCreateTemplate }) => {
    ...
    const reset = () => {
        ...
        setAutoPreset(null); setAutoCustomName('');
        setAutoFormat(''); setAutoStart(''); setAutoEnd('');
    };
    ...
    const headerInfo = useMemo(() => {
        ...
        if (mode === 'auto') {
            ...
            const [title, sub] = titles[step] ?? titles[titles.length - 1];
            return { title, sub, label: `Step ${step + 1} of ${AUTO_STEPS.length}` };
        }
        const titles = [
        ...
        const [title, sub] = titles[step] ?? titles[titles.length - 1];
        return { title, sub, label: `Step ${step + 1} of ${MANUAL_STEPS.length}` };
    }, [mode, step]);
    ...
    const canNext = useMemo(() => {
        ...
        return false;
    }, [mode, step, autoPreset, autoCustomName, autoFormat, autoStart, autoEnd, autoNp, ws, manualNp]);
                    ...
                    <>
                        ...
                        <Box sx={{ px: 3, pb: 3, display: 'flex', justifyContent: 'flex-end', borderTop: `0.5px solid ${alpha('#000', 0.08)}`, mx: 3, pt: 2 }}>
                            <Button onClick={handleClose} variant="outlined"
                                    sx={{ color: SLATE, borderColor: alpha('#000', 0.18), textTransform: 'none', fontWeight: 600, fontSize: '0.82rem', borderRadius: '8px', px: 2, py: 0.75, '&:hover': { bgcolor: alpha('#000', 0.04), borderColor: alpha('#000', 0.3) } }}>
                                Cancel
                            </Button>
                        </Box>
                    </>
                ...
                {!!mode && (
                    <>
                        ...
                        <Box sx={{ px: 3, pb: 3 }}>
                            {/* AUTO steps */}
                            {mode === 'auto' && step === 0 && (
                                <AutoStepTemplate selectedPreset={autoPreset} customName={autoCustomName}
                                                  onSelectPreset={handleSelectPreset} onCustomName={setAutoCustomName} />
                            )}
                            {mode === 'auto' && step === 1 && (
                            ...
                            {mode === 'auto' && step === 2 && autoPreset && (
                                <AutoStepReview preset={autoPreset} format={autoFormat}
                                                startMonth={autoStart} endMonth={autoEnd} customName={autoCustomName} />
                            )}
