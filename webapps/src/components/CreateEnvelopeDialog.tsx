...
import {
    ...
    IconButton, LinearProgress, Slide, Typography,
} from '@mui/material';
import { TransitionProps } from '@mui/material/transitions';
import { X, ChevronLeft, ChevronRight, CheckCircle, Link } from 'lucide-react';

import {
    MAROON, MAROON_DARK, ENVELOPE_COLORS,
    BLANK_FORM, BLANK_PAYMENT_INFO, buildSteps,
    NewEnvelopeForm, PaymentInfo,
} from '../utils/EnvelopeStepTypes';

import StepMode         from './envelopeSteps/StepMode';
import StepType         from './envelopeSteps/StepType';
import StepName         from './envelopeSteps/StepName';
import StepPaymentInfo  from './envelopeSteps/StepPaymentInfo';
import StepAmount       from './envelopeSteps/StepAmount';
import StepTimeline     from './envelopeSteps/StepTimeline';
import StepContribution from './envelopeSteps/StepContribution';
import StepReview       from './envelopeSteps/StepReview';

export type { NewEnvelopeForm, PaymentInfo };
...
// ── Finalise paymentInfo before submitting ────────────────────────────────────
function finalisePaymentInfo(f: NewEnvelopeForm): PaymentInfo | null {
    if (f.envelopeType !== 'PAYOFF') return null;
    const pi           = f.paymentInfo ?? BLANK_PAYMENT_INFO();
    const totalAmount  = pi.totalAmount || (f.targetAmount !== '' ? Number(f.targetAmount) : 0);
    const firstPayAmt  = pi.firstPaymentAmount || (pi.numberOfPayments > 0
        ? Math.round((totalAmount / pi.numberOfPayments) * 100) / 100
        : 0);
    const endDate      = pi.endDate || (() => {
        if (!pi.firstPaymentDate) return '';
        const d = new Date(pi.firstPaymentDate);
        d.setMonth(d.getMonth() + pi.totalMonths);
        return d.toISOString().split('T')[0];
    })();
    return { ...pi, totalAmount, firstPaymentAmount: firstPayAmt, endDate };
}
...
const CreateEnvelopeDialog: React.FC<CreateEnvelopeDialogProps> = ({ open, onClose, onSubmit }) => {
    ...
    const STEPS = useMemo(() => buildSteps(form.envelopeType), [form.envelopeType]);
    ...
    const reset = () => {
        setStep(0); setIsLinked(false);
        setForms([BLANK_FORM()]); setActiveIdx(0);
    };
    ...
    const canAdvance = useMemo(() => {
        ...
        switch (id) {
            ...
            case 'type':         return !!form.envelopeType;
            case 'name':         return form.envelopeName.trim().length >= 2;
            case 'payment': {
                const pi = form.paymentInfo;
                return !!pi && pi.merchant.trim().length > 0 && !!pi.firstPaymentDate;
            }
            case 'amount':       return form.targetAmount !== '' && Number(form.targetAmount) > 0;
            case 'timeline':     return true;
            case 'contribution': return !!form.contributionMode;
    ...
    const handleSubmit = async () => {
        setSubmitting(true);
        try {
            const finalForms = forms.map(f => ({ ...f, paymentInfo: finalisePaymentInfo(f) }));
            await onSubmit(isLinked ? finalForms : finalForms[0], isLinked);
            reset();
            onClose();
    ...
    const accentColor   = form.envelopeType ? ENVELOPE_COLORS[form.envelopeType] : MAROON;
        ...
        <Dialog open={open} onClose={handleClose} TransitionComponent={SlideUp}
            ...
            <Box sx={{
                ...
                <Box sx={{ display: 'flex', gap: 0.75, mb: 1.75, flexWrap: 'wrap' }}>
                    {STEPS.map((s, i) => (
                        <Box key={s.id} onClick={() => i < step && setStep(i)} sx={{
            ...
            <DialogContent sx={{ px: 3, py: 3, overflowY: 'auto' }}>
                {currentStepId === 'mode' && (
                    <StepMode
                        isLinked={isLinked} linkCount={linkCount}
                        onChange={(linked, count) => {
                            setIsLinked(linked);
                            setLinkCount(count);
                            setForms(linked
                                ? Array.from({ length: count }, () => BLANK_FORM())
                                : [forms[0] ?? BLANK_FORM()]);
                            setActiveIdx(0);
                        }}
                    />
                )}
                {currentStepId === 'type'         && <StepType         form={form} update={update} />}
                {currentStepId === 'name'         && <StepName         form={form} update={update} />}
                {currentStepId === 'payment'      && <StepPaymentInfo  form={form} update={update} />}
                {currentStepId === 'amount'       && <StepAmount       form={form} update={update} />}
                {currentStepId === 'timeline'     && <StepTimeline     form={form} update={update} />}
                {currentStepId === 'contribution' && <StepContribution form={form} update={update} />}
                {currentStepId === 'review'       && (
                    <StepReview
                        form={form} update={update} isLinked={isLinked}
                ...
                {step < STEPS.length - 1 ? (
                    <Button onClick={handleNext} disabled={!canAdvance} endIcon={<ChevronRight size={16} />}
                            ...
                            sx={{ borderRadius: '8px', textTransform: 'none', fontWeight: 700,
                                bgcolor: accentColor, '&:hover': { bgcolor: alpha(accentColor, 0.85) },
...
// } from 'lucide-react';
// import {PaymentInfo} from "../config/Types";
//
// // ── Design tokens ────────────────────────────────────────────────────────────
// const MAROON      = '#6b1a1a';
