...
import {
    ...
    Divider,
    FormControl,
    InputLabel, Select, MenuItem, SelectChangeEvent, FormControlLabel, FormLabel, RadioGroup, Radio,
    Fade, Alert
} from '@mui/material';
...
// ── Maroon theme matching the rest of the app ─────────────────────────────────
    ...
    components: {
        ...
        MuiButton: {
            styleOverrides: {
                ...
                text: {
                    color: '#800000',
                    '&:hover': { backgroundColor: 'rgba(128,0,0,0.06)' },
...
// Subtle maroon tint used for section backgrounds
const SECTION_BG  = 'rgba(128,0,0,0.04)';
const SECTION_BDR = '1px solid rgba(128,0,0,0.12)';
...
const BudgetQuestionnaireForm: React.FC<BudgetQuestionnaireProps> = ({ onSubmit, skipHistoricalData = false }) => {
    ...
    const [budgetData, setBudgetData] = useState<BudgetQuestions>({
        historicalData: { previousIncome: 0, hadPreviousBudget: false },
        budgetType: '',
        monthlyIncome: 0,
        expenseCategories: [],
        financialGoal: {
            budgetId: 0, goalName: '', goalDescription: '', goalType: '',
            targetAmount: 0, monthlyAllocation: 0, currentSavings: 0,
            savingsFrequency: '', status: ''
        },
    ...
    const [newCategory, setNewCategory] = useState<BudgetCategory>({
        budgetId: 0, categoryName: '', allocatedAmount: 0, monthlySpendingLimit: 0,
        currentSpending: 0, isFixedExpense: false, isActive: true, priority: 0,
    });
    const [notification, setNotification] = useState<{ message: string; severity: 'success' | 'error' | 'warning'; show: boolean }>({
    ...
    const clearNotification = () => setNotification(p => ({ ...p, show: false }));
    ...
    const handleHistoricalDataChange = (field: keyof HistoricalBudgetData, value: any) =>
        setBudgetData(prev => ({ ...prev, historicalData: { ...prev.historicalData!, [field]: value } }));

    const handleSavingsGoalDataChange    = (data: SavingsGoalData)    => setBudgetData(p => ({ ...p, savingsGoalData: data }));
    const handleDebtPayoffDataChange     = (data: DebtPayoffData)     => setBudgetData(p => ({ ...p, debtPayoffData: data }));
    const handleSpendingControlDataChange = (data: SpendingControlData) => setBudgetData(p => ({ ...p, spendingControlData: data }));

    const handleBudgetGoalRegistration = async (budgetId: number) => {
        try {
            let goalData: any;
            switch (budgetData.budgetType) {
                case "Saving for a goal":      if (!budgetData.savingsGoalData)  { console.error('Missing savings data'); return null; }  goalData = budgetData.savingsGoalData;  break;
                case "Paying off debt":        if (!budgetData.debtPayoffData)   { console.error('Missing debt data');    return null; }  goalData = budgetData.debtPayoffData;   break;
                case "Controlling spending":   if (!budgetData.spendingControlData) { console.error('Missing spending data'); return null; } goalData = budgetData.spendingControlData; break;
                default: console.error('Invalid budget type'); return null;
            }
            const res = await budgetGoalsService.createBudgetGoal(budgetId, budgetData, goalData);
            return res.data;
        } catch (error) { console.error('Error saving budget goals:', error); throw error; }
    };

    const handleBudgetCategoriesRegistration = async (budgetId: number) => {
        try {
            if (!budgetData.spendingControlData?.categories) { console.error('Missing spending data'); return null; }
            const cats: BudgetCategory[] = budgetData.spendingControlData.categories.map(c => ({
                budgetId, categoryName: c.name, allocatedAmount: c.spendingLimit,
                monthlySpendingLimit: c.spendingLimit, currentSpending: c.currentSpending,
                isFixedExpense: false, isActive: true, priority: c.reductionPriority,
            }));
            if (cats.length === 0) { console.warn('No categories to register'); return null; }
            const res = await budgetCategoriesService.createBudgetCategory(cats);
            return res.data;
        } catch (error) { console.error('Error saving categories:', error); throw error; }
    };
    ...
    const handleSubmit = async () => {
        try {
            ...
            const startDate: [number, number, number] = [currentDate.getFullYear(), 1, 1];
            const endDate:   [number, number, number] = [currentDate.getFullYear(), 12, 31];
            ...
            switch (budgetData.budgetType) {
                case 'Saving for a goal':
                    if (!budgetData.savingsGoalData) throw new Error('Savings Goal data is missing');
                    budgetGoals = { budgetId: 0, goalName: budgetData.savingsGoalData.goalName, goalDescription: budgetData.savingsGoalData.goalDescription || 'Savings goal', goalType: 'SAVINGS', targetAmount: budgetData.savingsGoalData.targetAmount, monthlyAllocation: budgetData.savingsGoalData.monthlyAllocation, currentSavings: budgetData.savingsGoalData.currentSavings || 0, savingsFrequency: budgetData.savingsGoalData.savingsFrequency || 'monthly', status: 'ACTIVE' };
                    budgetDescription += ` for ${budgetData.savingsGoalData.goalName}`; break;
                case 'Paying off debt':
                    if (!budgetData.debtPayoffData) throw new Error('Debt payoff data is missing');
                    budgetGoals = { budgetId: 0, goalName: 'Debt Reduction', goalDescription: 'Paying off outstanding debts', goalType: 'DEBT_REDUCTION', targetAmount: budgetData.debtPayoffData.debts.reduce((s, d) => s + d.amount, 0), monthlyAllocation: budgetData.debtPayoffData.debts.reduce((s, d) => s + d.allocation, 0), currentSavings: 0, savingsFrequency: 'MONTHLY', status: 'ACTIVE' };
                    budgetDescription += ` for debt repayment`; break;
                case 'Controlling spending':
                    if (!budgetData.spendingControlData) throw new Error('Spending control data is missing');
                    budgetGoals = { budgetId: 0, goalName: 'Spending Control', goalDescription: 'Manage and reduce spending', goalType: 'SPENDING_CONTROL', targetAmount: budgetData.spendingControlData.categories.reduce((s, c) => s + c.spendingLimit, 0), monthlyAllocation: 0, currentSavings: 0, savingsFrequency: 'MONTHLY', status: 'ACTIVE' };
                    budgetDescription += ` for spending control`; break;
                default:
                    budgetGoals = budgetData.financialGoal;
            }

            const budgetRegistration: BudgetRegistration = {
                userId, budgetName, budgetDescription, budgetPeriod: Period.MONTHLY, budgetMode, budgetGoals,
                budgetYear: startDate[0], budgetStartDate: startDate, budgetEndDate: endDate,
                budgetDateRanges, totalIncomeAmount: budgetData.monthlyIncome * numberOfMonths,
                numberOfMonths, totalBudgetsNeeded: numberOfMonths,
                previousIncomeAmount: budgetData.historicalData?.previousIncome || 0,
                previousBudgetName: budgetData.historicalData?.hadPreviousBudget ? `${startDate[0] - 1} ${budgetData.historicalData.previousBudgetType} Budget` : '',
            ...
            if (!setupSuccess) throw new Error('Budget setup process failed');

            setNotification({ message: 'Budget created successfully!', severity: 'success', show: true });
            onSubmit(budgetData);
            navigate('/');
        } catch (error) {
            let msg = 'Failed to create budget';
            if (error instanceof Error) msg += `: ${error.message}`;
            setNotification({ message: msg, severity: 'error', show: true });
        }
    };

    const canProceed = () => {
        const step = skipHistoricalData ? activeStep + 1 : activeStep;
        switch (step) {
            case 0: return true;
            case 1: return budgetData.budgetType !== '';
            case 2: return budgetData.monthlyIncome > 0;
            case 3:
                if (budgetData.budgetType === 'Saving for a goal')    return !!(budgetData.savingsGoalData?.goalName && budgetData.savingsGoalData.targetAmount > 0);
                if (budgetData.budgetType === 'Paying off debt')       return !!(budgetData.debtPayoffData?.debts.length);
                if (budgetData.budgetType === 'Controlling spending')  return !!(budgetData.spendingControlData?.categories.length);
                return false;
            default: return true;
        }
    };

    const categories = budgetData.spendingControlData?.categories || [];
    const isLastStep = activeStep === (skipHistoricalData ? 3 : 4);
                ...
                <Container maxWidth="md">
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', mb: 3 }}>
                        <Box
                            ...
                            alt="Budget Buddy"
                            sx={{ width: 220, height: 'auto', objectFit: 'contain', backgroundColor: 'transparent' }}
                        />
                    </Box>

                    {notification.show && (
                        <Fade in={notification.show}>
                            <Alert severity={notification.severity} sx={{ mb: 3, borderRadius: 2 }} onClose={clearNotification}>
                                {notification.message}
                            </Alert>
                        </Fade>
                    )}

                    <Paper elevation={4} sx={{ borderRadius: 3, p: { xs: 3, md: 5 }, backgroundColor: '#ffffff' }}>
                        {/* Header */}
                        <Typography variant="h5" align="center" sx={{ mb: 4, color: '#800000' }}>
                            ...
                            {!skipHistoricalData && activeStep === 0 && (
                                <Fade in>
                                    <Box>
                                        ...
                                        <TextField
                                            fullWidth label="What was your annual income last year?"
                                            type="number"
                                        ...
                                        <FormControl component="fieldset" sx={{ mb: 3, width: '100%' }}>
                                            <FormLabel sx={{ color: 'text.secondary', mb: 1 }}>Did you have a budget last year?</FormLabel>
                                            <RadioGroup row
                                            ...
                                            <Fade in>
                                                <Box>
                                                    ...
                                                    {budgetData.historicalData?.previousBudgetType === 'Saving' && (
                                                        <TextField fullWidth label="How much did you manage to save?" type="number"
                                                                   InputProps={{ startAdornment: <Box component="span" sx={{ mr: 0.5, color: 'text.secondary' }}>$</Box> }}
                                                    ...
                                                    <FormControl component="fieldset" sx={{ mb: 3, width: '100%' }}>
                                                        <FormLabel sx={{ color: 'text.secondary', mb: 1 }}>Are your financial goals similar to last year?</FormLabel>
                                                        <RadioGroup row
                                ...
                                <Fade in>
                                    <Box>
                                        ...
                                        <Grid container spacing={3}>
                                            {(["Saving for a goal", "Paying off debt", "Controlling spending", "Building Emergency Fund"] as BudgetType[]).map(type => (
                            ...
                            {((skipHistoricalData && activeStep === 1) || (!skipHistoricalData && activeStep === 2)) && (
                                <Fade in>
                                    <Box>
                                        ...
                                        <Box sx={{ p: 4, borderRadius: 3, bgcolor: SECTION_BG, border: SECTION_BDR, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                            <MonetizationOn sx={{ fontSize: 56, color: 'rgba(128,0,0,0.3)', mb: 2 }} />
                                            <TextField
                                                fullWidth label="Monthly Income" type="number"
                                                placeholder="Enter your monthly income"
                                                InputProps={{
                                                    startAdornment: <Box component="span" sx={{ mr: 0.5, color: 'text.secondary', fontSize: '1.1rem' }}>$</Box>,
                                                    sx: { fontSize: '1.1rem' },
                                                }}
                            ...
                            {((skipHistoricalData && activeStep === 3) || (!skipHistoricalData && activeStep === 4)) && (
                                <Fade in>
                                    <Box>
                                        ...
                                        <Box sx={{ p: 3, borderRadius: 2, bgcolor: SECTION_BG, border: SECTION_BDR, mb: 3 }}>
                                            <List sx={{ p: 0 }}>

                                                {/* Historical data */}
                                                {budgetData.historicalData && (
                                                    <ListItem sx={{ bgcolor: '#ffffff', mb: 2, borderRadius: 2, flexDirection: 'column', alignItems: 'flex-start', p: 2 }}>
                                                        <Typography variant="subtitle1" color="primary" fontWeight={600} sx={{ mb: 1 }}>Previous Year's Information</Typography>
                                                        <Typography variant="body2">Previous Annual Income: ${budgetData.historicalData.previousIncome?.toFixed(2)}</Typography>
                                                        <Typography variant="body2">Had Previous Budget: {budgetData.historicalData.hadPreviousBudget ? 'Yes' : 'No'}</Typography>
                                                        {budgetData.historicalData.hadPreviousBudget && (
                                                            <>
                                                                <Typography variant="body2">Previous Focus: {budgetData.historicalData.previousBudgetType}</Typography>
                                                                {budgetData.historicalData.previousGoalsSimilar === false && (
                                                                    <Typography variant="body2">Changes This Year: {budgetData.historicalData.previousGoalsDifference}</Typography>
                                                                )}
                                                            </>
                                                        )}
                                                    </ListItem>
                                                ...
                                                {/* Budget type */}
                                                <ListItem sx={{ bgcolor: '#ffffff', mb: 2, borderRadius: 2, p: 2 }}>
                                                    <ListItemText
                                                        primary={<Typography variant="subtitle1" color="primary" fontWeight={600}>Budget Type</Typography>}
                                                        secondary={
                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.5 }}>
                                                                {budgetData.budgetType && budgetTypeIcons[budgetData.budgetType as BudgetType]}
                                                                <Typography>{budgetData.budgetType}</Typography>
                                                            </Box>
                                                        }
                                                    />
                                                </ListItem>
                                                ...
                                                <ListItem sx={{ bgcolor: '#ffffff', mb: 2, borderRadius: 2, p: 2 }}>
                                                    <ListItemText
                                                        primary={<Typography variant="subtitle1" color="primary" fontWeight={600}>Monthly Income</Typography>}
                                                        secondary={<Typography sx={{ mt: 0.5, fontWeight: 600, fontSize: '1.05rem' }}>${budgetData.monthlyIncome.toFixed(2)}</Typography>}
                                                    />
                                                </ListItem>
                                                ...
                                                {budgetData.budgetType === 'Saving for a goal' && budgetData.savingsGoalData && (
                                                    <ListItem sx={{ bgcolor: '#ffffff', mb: 2, borderRadius: 2, flexDirection: 'column', alignItems: 'flex-start', p: 2 }}>
                                                        <Typography variant="subtitle1" color="primary" fontWeight={600} sx={{ mb: 1 }}>Savings Goal</Typography>
                                                        <Grid container spacing={2}>
                                                            <Grid item xs={12} sm={6}>
                                                                <Typography variant="body2" fontWeight={600}>Goal: {budgetData.savingsGoalData.goalName}</Typography>
                                                                <Typography variant="body2">Description: {budgetData.savingsGoalData.goalDescription}</Typography>
                                                            </Grid>
                                                            <Grid item xs={12} sm={6}>
                                                                <Typography variant="body2">Target: <strong>${budgetData.savingsGoalData.targetAmount.toFixed(2)}</strong></Typography>
                                                                <Typography variant="body2">Current Savings: ${budgetData.savingsGoalData.currentSavings.toFixed(2)}</Typography>
                                                                <Typography variant="body2">Monthly Allocation: ${budgetData.savingsGoalData.monthlyAllocation?.toFixed(2) || 'N/A'}</Typography>
                                                                {budgetData.savingsGoalData.targetDate && (
                                                                    <Typography variant="body2">Target Date: {new Date(budgetData.savingsGoalData.targetDate).toLocaleDateString()}</Typography>
                                                                )}
                                                            </Grid>
                                                        </Grid>
                                                    </ListItem>
                                                ...
                                                {budgetData.budgetType === 'Paying off debt' && budgetData.debtPayoffData && (
                                                    <ListItem sx={{ bgcolor: '#ffffff', mb: 2, borderRadius: 2, flexDirection: 'column', alignItems: 'flex-start', p: 2 }}>
                                                        <Typography variant="subtitle1" color="primary" fontWeight={600} sx={{ mb: 1 }}>Debt Payoff Plan</Typography>
                                                        <Box sx={{ maxHeight: 200, overflow: 'auto', width: '100%', mb: 1 }}>
                                                            {budgetData.debtPayoffData.debts.map((debt, i) => (
                                                                <Box key={i} sx={{ mb: 1, p: 1.5, bgcolor: SECTION_BG, borderRadius: 1 }}>
                                                                    <Typography variant="body2" fontWeight={600}>{debt.type}</Typography>
                                                                    <Grid container spacing={1}>
                                                                        <Grid item xs={6}><Typography variant="body2">Total: ${debt.amount.toFixed(2)}</Typography></Grid>
                                                                        <Grid item xs={6}><Typography variant="body2">Monthly: ${debt.allocation.toFixed(2)}</Typography></Grid>
                                                                    </Grid>
                                                                </Box>
                                                            ))}
                                                        </Box>
                                                        ...
                                                        <Grid container spacing={2}>
                                                            <Grid item xs={6}><Typography variant="body2" fontWeight={600}>Total Debt: <Box component="span" sx={{ color: 'error.main' }}>${budgetData.debtPayoffData.debts.reduce((s, d) => s + d.amount, 0).toFixed(2)}</Box></Typography></Grid>
                                                            <Grid item xs={6}><Typography variant="body2" fontWeight={600}>Monthly: <Box component="span" sx={{ color: 'primary.main' }}>${budgetData.debtPayoffData.debts.reduce((s, d) => s + d.allocation, 0).toFixed(2)}</Box></Typography></Grid>
                                                        </Grid>
                                                    </ListItem>
                                                )}
                                                ...
                                                {budgetData.budgetType === 'Controlling spending' && budgetData.spendingControlData && (
                                                    <ListItem sx={{ bgcolor: '#ffffff', mb: 2, borderRadius: 2, flexDirection: 'column', alignItems: 'flex-start', p: 2 }}>
                                                        <Typography variant="subtitle1" color="primary" fontWeight={600} sx={{ mb: 1 }}>Spending Control Plan</Typography>
                                                        <Box sx={{ maxHeight: 200, overflow: 'auto', width: '100%', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 2 }}>
                                                            {categories.length > 0 ? (
                                                                <List disablePadding>
                                                                    {categories.map((cat, i) => (
                                                                        <ListItem key={i} dense divider={i < categories.length - 1} sx={{ p: 1.5 }}>
                                                                            <ListItemText
                                                                                primary={<Typography variant="body2" fontWeight={600}>{cat.name}</Typography>}
                                                                                secondary={
                                                                                    <Grid container spacing={1} sx={{ mt: 0.25 }}>
                                                                                        <Grid item xs={6}><Typography variant="caption">Current: ${cat.currentSpending}</Typography></Grid>
                                                                                        <Grid item xs={6}><Typography variant="caption">Limit: ${cat.spendingLimit}</Typography></Grid>
                                                                                        <Grid item xs={12}><Typography variant="caption">Priority: {cat.reductionPriority}</Typography></Grid>
                                                                                    </Grid>
                                                                                }
                                                                            />
                                                                        </ListItem>
                                                                ...
                                                                <Typography sx={{ p: 2 }} variant="body2" color="text.secondary">No spending categories added.</Typography>
                                                            )}
                                                        </Box>
                                                        <Typography variant="body2" fontWeight={600} sx={{ mt: 1.5 }}>
                                                            Total Monthly Limit:{' '}
                                                            <Box component="span" sx={{ color: 'primary.main' }}>
                                                                ${categories.reduce((s, c) => s + c.spendingLimit, 0).toFixed(2)}
                                                            </Box>
                                                        </Typography>
                                                    </ListItem>
                                                )}
                        ...
                        {/* ── Navigation row ─────────────────────────────────────────────── */}
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', pt: 3, borderTop: '1px solid rgba(0,0,0,0.08)' }}>
                            <Button
                                variant="text"
                                disabled={activeStep === 0}
                                onClick={handleBack}
                                startIcon={<ChevronLeft size={18} />}
                                sx={{ color: activeStep === 0 ? 'rgba(0,0,0,0.38)' : '#800000' }}
                            >
                                Back
                            </Button>
                            ...
                            {!isLastStep && (
                                <Button
                                    ...
                                    disabled={!canProceed()}
                                >
                                    {activeStep === (skipHistoricalData ? 2 : 3) ? 'Review' : 'Next'}
                                </Button>
                            )}
                        </Box>
                    </Paper>
