...
import {
    Dialog, DialogContent, DialogActions, Button, List, ListItem,
    ListItemButton, ListItemText, TextField, Box, Typography,
    Divider, IconButton, Chip, CircularProgress, InputAdornment,
    Card, alpha, ListItemSecondaryAction, Tooltip,
} from '@mui/material';
...
const CategoryDialog: React.FC<CategoryDialogProps> = ({
                                                           open, onClose, currentCategory = '', transactionId,
                                                           availableCategories, customCategories = [],
                                                           onSave, onAddCustomCategory, onDeleteCustomCategory,
                                                       }) => {
            ...
            if (open && systemCategories.length === 0) {
                ...
                try {
                    ...
                    const fetchPromise = (async () => {
                        ...
                        setSystemCategories(sysCategories.map(cat => cat.category));
    ...
    const allCategories = React.useMemo(() => {
        ...
        userCustomCategories.forEach(cat => { if (cat.category) categorySet.add(cat.category); });
    ...
    const filteredCategories = React.useMemo(() => {
        ...
        return allCategories.filter(cat => cat.toLowerCase().includes(searchQuery.toLowerCase()));
    }, [allCategories, searchQuery]);

    const isSystemCategory = (category: string) => systemCategories.includes(category);

    const handleAddCustomCategory = async () => {
        if (!customCategory.trim()) return;
        const newCategory = customCategory.trim();
        if (allCategories.includes(newCategory)) return;
        try {
            ...
            setCustomCategory('');
            if (onAddCustomCategory) await onAddCustomCategory(newCategory);
        } catch (error) {
            console.error('Error adding custom category:', error);
            setLocalCustomCategories(prev => prev.includes(newCategory) ? prev : [...prev, newCategory]);
            setSelectedCategory(newCategory);
            setCustomCategory('');
        }
    };
    ...
    const handleDeleteCustomCategory = async (category: string, event: React.MouseEvent) => {
        ...
        try {
            ...
            if (onDeleteCustomCategory) await onDeleteCustomCategory(category);
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            await onSave({
                ...
                isCustomCategory: !isSystemCategory(selectedCategory),
            });
            onClose();
        } catch (error) {
        ...
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth
            ...
            <Box sx={{
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <CategoryIcon />
                    <Typography variant="h6" fontWeight={600}>Categorize Transaction</Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Box sx={{ display: 'flex', gap: 1 }}>
                        <Chip icon={<SettingsIcon sx={{ fontSize: 14 }} />} label="System" size="small"
                              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: 'white', fontWeight: 600, '& .MuiChip-icon': { color: 'white' } }} />
                        <Chip icon={<LabelIcon sx={{ fontSize: 14 }} />} label="Custom" size="small"
                              sx={{ bgcolor: alpha(tealColor, 0.9), color: 'white', fontWeight: 600, '& .MuiChip-icon': { color: 'white' } }} />
                    </Box>
            ...
            <DialogContent sx={{ p: 0 }}>
                {loadingCategories ? (
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', py: 8, gap: 2 }}>
                        ...
                        <Typography variant="body1" color="text.secondary" fontWeight={500}>Loading categories...</Typography>
                    </Box>
                ) : (
                    <Box>
                        {/* Category Selection */}
                        <Box sx={{ p: 3 }}>
                            <Typography variant="subtitle1" fontWeight={600} sx={{ mb: 2 }}>Select Category</Typography>
                                    ...
                                    {filteredCategories.length === 0 ? (
                                        ...
                                        return (
                                            <ListItem key={category} sx={{
                                                ...
                                                <ListItemButton selected={isSelected} onClick={() => setSelectedCategory(category)}
                                                                sx={{
                                                                    py: 1.5, transition: 'all 0.2s',
                                                                    '&.Mui-selected': { backgroundColor: alpha(maroonColor, 0.1) },
                                                                    '&:hover': { backgroundColor: alpha(isSystem ? '#ccc' : tealColor, 0.1) },
                                                                }}>
                                                    <ListItemText primary={
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                            <Typography variant="body1" fontWeight={isSelected ? 600 : 500}
                                                                        sx={{ color: isSelected ? maroonColor : 'text.primary' }}>
                                                                {category}
                                                            </Typography>
                                                            <Chip label={isSystem ? 'System' : 'Custom'} size="small" sx={{
                                                    ...
                                                    <ListItemSecondaryAction>
                                                        <Tooltip title="Delete custom category" arrow>
                                                            <IconButton edge="end" size="small"
                                                                        ...
                                                                        sx={{ color: '#dc2626', mr: 1, '&:hover': { bgcolor: alpha('#dc2626', 0.1) } }}>
                        ...
                        {/* Add Custom Category */}
                        <Box sx={{ p: 3 }}>
                            <Typography variant="subtitle1" fontWeight={600} sx={{ mb: 2 }}>Add Custom Category</Typography>
                            <Card sx={{ p: 2, bgcolor: alpha(tealColor, 0.05), border: `1px solid ${alpha(tealColor, 0.2)}`, borderRadius: 2 }}>
                                <Box sx={{ display: 'flex', gap: 1.5 }}>
                                    <TextField fullWidth size="small" label="Custom Category Name"
                                               value={customCategory} onChange={(e) => setCustomCategory(e.target.value)}
                                               placeholder="e.g., Home Improvement, Pet Care"
                                               onKeyPress={(e) => { if (e.key === 'Enter' && customCategory.trim()) handleAddCustomCategory(); }}
                                    />
                                    <Button variant="contained" onClick={handleAddCustomCategory}
                                            disabled={!customCategory.trim()} startIcon={<AddIcon />}
                                            sx={{ minWidth: 100, textTransform: 'none', fontWeight: 600, bgcolor: tealColor, '&:hover': { bgcolor: '#0f766e' } }}>
                                        Add
                                    </Button>
                                </Box>
                            </Card>
            ...
            <DialogActions sx={{ p: 3, pt: 2 }}>
                ...
                <Button onClick={handleSave} variant="contained"
                        disabled={!selectedCategory || saving} startIcon={<SaveIcon />}
                        sx={{ textTransform: 'none', fontWeight: 600, minWidth: 120, px: 4, bgcolor: maroonColor, '&:hover': { bgcolor: '#a00000' } }}>
                    {saving ? <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <CircularProgress size={16} color="inherit" />Saving...
                    </Box> : 'Save Category'}
                </Button>
            </DialogActions>
        </Dialog>
