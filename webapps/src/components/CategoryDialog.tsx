...
const CategoryDialog: React.FC<CategoryDialogProps> = ({
    ...
    const allCategories = React.useMemo(() => {
        ...
        // Add user custom categories from database
        ...
        // Add available categories (legacy support)
        if (availableCategories) {
            availableCategories.forEach(cat => categorySet.add(cat));
        }

        // Add custom categories from props (legacy support)
        if (customCategories) {
            customCategories.forEach(cat => categorySet.add(cat));
        }

        // Add locally added custom categories (not yet saved)
    ...
    const handleDeleteCustomCategory = async (category: string, event: React.MouseEvent) => {
        ...
        try {
            // Find the category in userCustomCategories
            ...
            if (userCategory) {
                ...
                // It's a locally added category that hasn't been saved yet
    ...
    const isCustomCategoryLocal = (category: string) => {
        ...
        // Check if it's in locally added categories
        const isLocalCategory = localCustomCategories.includes(category);

        // Check if it's in legacy custom categories and not a system category
    ...
    const handleAddCustomCategory = async () => {
        ...
        try {
            ...
            // Optionally add to local state anyway for temporary use
            setLocalCustomCategories(prev => {
                if (prev.includes(newCategory)) {
                    return prev;
                }
