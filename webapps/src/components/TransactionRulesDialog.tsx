...
import {
    Dialog, DialogContent, Button, Box, Typography, IconButton,
    TextField, InputAdornment, Switch, Divider, alpha, Tooltip,
    Menu, MenuItem, CircularProgress, Collapse, Card, Stack,
    FormControlLabel, Checkbox, Select, MenuItem as MuiMenuItem,
...
interface TransactionRulesDialogProps {
    ...
    onAddRule: (rule: TransactionRule) => Promise<void>;
}

const PRIORITY_OPTIONS = [
    { value: 1, label: 'P1 — Merchant + Description + Extended + Amount' },
    { value: 2, label: 'P2 — Merchant + Amount' },
    { value: 3, label: 'P3 — Merchant + Min Amount' },
    { value: 4, label: 'P4 — Merchant + Max Amount' },
    { value: 5, label: 'P5 — Description + Merchant' },
    { value: 6, label: 'P6 — Merchant only' },
];
...
const emptyRule = (userId: number): TransactionRule => ({
    userId,
    categoryName: '',
    merchantRule: '',
    descriptionRule: '',
...
const TransactionRulesDialog: React.FC<TransactionRulesDialogProps> = ({
                                                                           open, onClose, rules, loading = false, userId, availableCategories,
                                                                           onDeleteRule, onToggleRule, onEditRule, onAddRule,
                                                                       }) => {
    ...
    const [newRule,     setNewRule]     = useState<TransactionRule>(emptyRule(userId));
    ...
    React.useEffect(() => {
        ...
        const fetchCategories = async () => {
            ...
            try {
                const [sysCategories, userCategories] = await Promise.all([
                    categoryService.getAllSystemCategories(),
                    userCategoryService.getCustomUserCategories(userId),
                ]);
                const categorySet = new Set<string>();
                sysCategories.forEach(cat => categorySet.add(cat.category));
                userCategories.forEach(cat => { if (cat.category) categorySet.add(cat.category); });
                setFetchedCategories(Array.from(categorySet).sort());
            } catch (error) {
    ...
    const handleCreateRule = async () => {
        if (!newRule.categoryName?.trim()) {
            setRuleError('Category name is required.');
            return;
        }
        if (!newRule.merchantRule?.trim()) {
            setRuleError('At least a merchant rule is required.');
            return;
        }
        setRuleError('');
        setSavingRule(true);
        try {
            await onAddRule({ ...newRule, userId });
            setLocalRules(prev => [...prev, newRule]);
            setNewRule(emptyRule(userId));
            setShowCreate(false);
        } catch (error) {
            setRuleError('Failed to save rule. Please try again.');
        ...
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth
            ...
            <Box sx={{
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    ...
                    <Box>
                        <Typography sx={{ fontWeight: 800, fontSize: '1rem', color: '#fff', lineHeight: 1.2 }}>Transaction Rules</Typography>
                        <Typography sx={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.7)', mt: 0.15 }}>Auto-categorization rules</Typography>
                    </Box>
                </Box>
            ...
            {/* Create Rule Section */}
            <Box sx={{ px: 2.5, py: 1.5, borderBottom: `1px solid ${alpha('#e0e0e0', 0.8)}`, bgcolor: '#fafafa', flexShrink: 0 }}>
                <Button
                    fullWidth
                    variant={showCreate ? 'contained' : 'outlined'}
                    startIcon={showCreate ? <ExpandLessIcon /> : <AddIcon />}
                    onClick={() => { setShowCreate(!showCreate); setRuleError(''); }}
                    sx={{
                        textTransform: 'none', fontWeight: 700, borderRadius: 2, py: 1,
                        ...(showCreate
                                ? { background: MAROON_GRAD, '&:hover': { background: MAROON_GRAD, filter: 'brightness(1.1)' } }
                                : { borderColor: MAROON, color: MAROON, '&:hover': { bgcolor: alpha(MAROON, 0.05), borderColor: MAROON } }
                        )
                    }}
                >
                    {showCreate ? 'Cancel' : 'Create New Rule'}
                </Button>

                <Collapse in={showCreate}>
                    <Card sx={{ mt: 2, p: 2.5, border: `1px solid ${alpha(MAROON, 0.15)}`, borderRadius: 2, bgcolor: 'white' }}>
                        <Typography sx={{ fontWeight: 700, fontSize: '0.925rem', color: MAROON, mb: 2 }}>New Rule</Typography>
                        <Stack spacing={2}>
                            {/* Category */}
                            <FormControl fullWidth size="small">
                                ...
                                >
                                    {loadingCategories
                                        ? <MuiMenuItem disabled>Loading...</MuiMenuItem>
                            ...
                            {/* Priority */}
                            <FormControl fullWidth size="small">
                                <InputLabel>Priority / Match Type *</InputLabel>
                                <Select
                                    value={newRule.priority ?? 6}
                                    label="Priority / Match Type *"
                            ...
                            {/* Merchant */}
                            <TextField fullWidth size="small" label="Merchant Rule *"
                                       value={newRule.merchantRule ?? ''}
                                       onChange={(e) => setNewRule(prev => ({ ...prev, merchantRule: e.target.value }))}
                                       placeholder="e.g., WINCO FOODS"
                                       InputProps={{ startAdornment: <InputAdornment position="start"><StorefrontIcon sx={{ fontSize: 18, color: '#bbb' }} /></InputAdornment> }}
                                       sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                            />

                            {/* Description */}
                            {[1, 5].includes(newRule.priority ?? 6) && (
                                <TextField fullWidth size="small" label="Description Rule"
                                           value={newRule.descriptionRule ?? ''}
                                           onChange={(e) => setNewRule(prev => ({ ...prev, descriptionRule: e.target.value }))}
                                           placeholder="e.g., Groceries"
                                           InputProps={{ startAdornment: <InputAdornment position="start"><DescriptionIcon sx={{ fontSize: 18, color: '#bbb' }} /></InputAdornment> }}
                                           sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                                />
                            )}
                            ...
                            {newRule.priority === 1 && (
                                <TextField fullWidth size="small" label="Extended Description Rule"
                                           value={newRule.extendedDescriptionRule ?? ''}
                                           onChange={(e) => setNewRule(prev => ({ ...prev, extendedDescriptionRule: e.target.value }))}
                                           placeholder="e.g., WinCo Foods #23"
                                           sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                                />
                            )}

                            {/* Amount Range */}
                            {[1, 2, 3, 4].includes(newRule.priority ?? 6) && (
                                <Box sx={{ display: 'flex', gap: 2 }}>
                                    <TextField fullWidth size="small" label="Min Amount" type="number"
                                               value={newRule.amountMin ?? 0}
                                               onChange={(e) => setNewRule(prev => ({ ...prev, amountMin: Number(e.target.value) }))}
                                               inputProps={{ step: 0.01, min: 0 }}
                                               InputProps={{ startAdornment: <InputAdornment position="start">$</InputAdornment> }}
                                               sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                                    />
                                    <TextField fullWidth size="small" label="Max Amount" type="number"
                                               value={newRule.amountMax ?? 0}
                                               onChange={(e) => setNewRule(prev => ({ ...prev, amountMax: Number(e.target.value) }))}
                                               inputProps={{ step: 0.01, min: 0 }}
                                               InputProps={{ startAdornment: <InputAdornment position="start">$</InputAdornment> }}
                                               sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                                    />
                            ...
                            <FormControlLabel
                                control={
                                    <Switch checked={newRule.isActive ?? true}
                                ...
                                label={<Typography variant="body2" fontWeight={600}>Active immediately</Typography>}
                            />

                            {ruleError && (
                                <Typography sx={{ fontSize: '0.825rem', color: '#dc2626', fontWeight: 600 }}>{ruleError}</Typography>
                            )}

                            <Button variant="contained" onClick={handleCreateRule} disabled={savingRule}
                                    startIcon={savingRule ? <CircularProgress size={16} color="inherit" /> : <AddIcon />}
                                    sx={{
                                        textTransform: 'none', fontWeight: 700, borderRadius: 2, alignSelf: 'flex-end',
                                        px: 3, background: MAROON_GRAD,
                                        '&:hover': { background: MAROON_GRAD, filter: 'brightness(1.1)' }
                                    }}>
            ...
            <Box sx={{ px: 2.5, py: 1.75, borderBottom: `1px solid ${alpha('#e0e0e0', 0.8)}`, bgcolor: '#fafafa', flexShrink: 0 }}>
                <TextField size="small" fullWidth placeholder="Search by category, merchant, or description…"
                           ...
                           sx={{ mb: 1.5, '& .MuiOutlinedInput-root': { borderRadius: 2, fontSize: '0.9rem', bgcolor: 'white', '&.Mui-focused fieldset': { borderColor: MAROON } }, '& .MuiInputBase-input': { py: '8px' } }}
                />
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                {loading ? (
                    ...
                    <Box sx={{ textAlign: 'center', py: 12, px: 3 }}>
                        ...
                        <Typography sx={{ fontSize: '0.875rem', color: '#aaa' }}>
                            {search ? `No matches for "${search}"` : 'Use the form above to create your first rule'}
                        </Typography>
                    </Box>
                    ...
                    return (
                        <Box key={rule.id ?? idx}>
                            <Box sx={{
                                ...
                                <Switch checked={rule.isActive ?? false} onChange={() => handleToggle(rule.id!, rule.isActive)} size="small"
                                        sx={{ flexShrink: 0, '& .MuiSwitch-switchBase.Mui-checked': { color: TEAL }, '& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track': { bgcolor: TEAL } }} />
                            ...
                            <Collapse in={isExpanded}>
                                <Box sx={{ px: 3, py: 2, borderLeft: `3px solid ${MAROON}`, borderBottom: `1px solid ${alpha('#e0e0e0', 0.6)}`, bgcolor: alpha(MAROON, 0.02) }}>
                                    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px 24px' }}>
                                        {[
                                            ...
                                            <Box key={f.label}>
                                                <Typography sx={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.07em', color: '#bbb', fontWeight: 700, mb: 0.3 }}>{f.label}</Typography>
                                                <Typography sx={{ fontSize: '0.875rem', fontWeight: 600, color: '#333' }}>{f.value}</Typography>
                                            </Box>
                                        ))}
                                    </Box>
                                </Box>
            ...
            <Box sx={{ px: 3, py: 1.75, borderTop: `1px solid ${alpha('#e0e0e0', 0.8)}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: '#fafafa', flexShrink: 0 }}>
                ...
                <Button onClick={onClose} variant="contained"
                        sx={{ textTransform: 'none', fontWeight: 700, borderRadius: 2, px: 3.5, py: 1, fontSize: '0.9rem', background: MAROON_GRAD, boxShadow: `0 4px 12px ${alpha(MAROON, 0.3)}`, '&:hover': { background: `linear-gradient(135deg, ${MAROON} 0%, ${MAROON_DARK} 100%)` } }}>
                    Done
                </Button>
            </Box>
            ...
            <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={() => { setMenuAnchor(null); setMenuRule(null); }}
                ...
                {menuRule && [
                    ...
                    <MenuItem key="delete" onClick={() => handleDelete(menuRule.id!)} disabled={deletingId === menuRule.id}
                        ...
                        {deletingId === menuRule.id ? <CircularProgress size={16} sx={{ color: '#dc2626' }} /> : <DeleteIcon sx={{ fontSize: 18 }} />}
                        Delete
                    </MenuItem>,
                ]}
            </Menu>
