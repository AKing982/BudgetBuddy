...
import com.app.budgetbuddy.workbench.TransactionParser;
...
public class CSVParserService {
    ...
    @Autowired
    public CSVParserService(List<TransactionParser> parsers)
    {
        this.parsers = parsers;
    }

    public List<TransactionCSV> parseCSV(MultipartFile file, String institution, Long userId)
    {
        if (file == null || file.isEmpty()) throw new DataException("File is empty");

        TransactionParser parserStrategy = parsers.stream()
                .filter(p -> p.supports(institution))
                .findFirst()
                .orElseThrow(() -> new DataException("Unsupported institution: " + institution));
