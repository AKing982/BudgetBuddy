...
{
    @Query("SELECT u FROM TransactionCategoryEntity u WHERE u.subBudget.budget.user.id =:id")
    List<TransactionCategoryEntity> findAllByUserId(@Param("id") Long id);

    @Query("SELECT u FROM TransactionCategoryEntity u WHERE u.subBudget.budget.user.id =:id AND u.isactive = true")
    List<TransactionCategoryEntity> findActiveCategoriesByUser(@Param("id") Long userId);

    @Query("SELECT u FROM TransactionCategoryEntity u WHERE u.subBudget.id =:id")
    List<TransactionCategoryEntity> findByBudgetId(@Param("id") Long budgetId);

    @Query("SELECT u FROM TransactionCategoryEntity u WHERE u.subBudget.id =:id AND u.startDate =:start AND u.endDate =:end")
    ...
    @Query("SELECT u FROM TransactionCategoryEntity u WHERE u.subBudget.budget.user.id =:id AND u.startDate =:startDate AND u.endDate =:endDate")
    ...
    @Query("SELECT SUM(u.budgetedAmount) FROM TransactionCategoryEntity u WHERE u.subBudget.budget.user.id =:id AND u.startDate =:start AND u.endDate =:end")
