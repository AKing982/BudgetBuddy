...
public interface SubBudgetRepository extends JpaRepository<SubBudgetEntity, Long>
{
    ...
    @Query("SELECT sb FROM SubBudgetEntity sb JOIN sb.budget b WHERE b.user.id =:userId AND sb.startDate >=:start AND sb.endDate <=:end ")
    List<SubBudgetEntity> findSubBudgetEntitiesByUserIdAndDateRange(@Param("userId") Long userId, @Param("start") LocalDate start, @Param("end") LocalDate end);
