...
public class DateRange implements Comparable<DateRange>
{
    ...
    public List<DateRange> splitIntoWeeks(){
        ...
        while (current.isBefore(endDate) || current.isEqual(endDate))
        {
            ...
            LocalDate rangeEnd = current.plusDays(6); // End of the week is 6 days after current

            // If rangeEnd exceeds endDate, clamp it to endDate
            if (rangeEnd.isAfter(endDate)) {
                rangeEnd = endDate;
            }

            // Add the weekly range to the list
            dateRanges.add(new DateRange(current, rangeEnd));

            // Move current to the next day after the current range
    ...
    public List<DateRange> splitIntoBiWeeks(){
        ...
        LocalDate current = startDate;
        while(!current.isAfter(endDate)){
            // Calculate the end date for the bi-weekly period
            LocalDate rangeEnd = current.plusWeeks(2).minusDays(1);

            // Ensure the rangeEnd does not exceed endDate
            if (rangeEnd.isAfter(endDate)) {
                rangeEnd = endDate;
            }

            // Add the bi-weekly range to the list
            DateRange biWeek = new DateRange(current, rangeEnd);
            System.out.println("Bi Week: " + biWeek.toString());
